...
char* StrCpy(char* dest, const char* source)
{
	...
	while (*dest++ = *source++);				// 1. 请解释本循环的执行过程：何时结束循环，为什么？指针如何变化、指针的目标如何变化？
												// 1A: dest字符串字符被source赋值为\0时结束循环。
												//     指针    ： 两指针持续自增直至source的\0被赋值到dest。
												//     指针目标： dest被source值覆盖。
	return temp;								// 2. 为什么要使用指针变量temp？此处能否改成return dest;？
												// 2A: 传回头位置， 后者传了目标字符串末尾
}
...
int StrCmp(const char* str1, char* str2)
{
	...
	unsigned char* q = (unsigned char*)str2;	// 4A: 避免大的字符码变负数反而比小的字符码小。
	while (*p && *q && *p == *q)				// 5. 有哪些情况可能导致本循环结束（最好能举例说明）？
		p++, q++;								// 5A: 任意一个字符到\0、两字符串在一处不相等。
